...
export default function ProductsPage() {
  ...
  const { data, isLoading }: any = useQuery({
    queryKey: ["products"],
    queryFn: productApi.getProducts as any,
  });

  const addMut = useMutation({
    mutationFn: () =>
      productApi.addProduct({ ...form, price: Number(form.price) }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["products"] });
      setShowAdd(false);
      setForm({ name: "", price: "", unit: "কেজি", emoji: "📦", category: "" });
    },
  ...
  return (
    <div className="p-4 animate-fade-in">
      ...
      {products.length === 0 ? (
        <EmptyState icon="📦" title="কোনো পণ্য নেই" />
      ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {EMOJIS.map((e) => (
                <button
                  ...
                  className={`text-2xl p-1 rounded-lg border ${form.emoji === e ? "border-teal-500 bg-teal-100 dark:bg-teal-900/30" : "border-transparent"}`}
                >
                  {e}
                </button>
              ))}
