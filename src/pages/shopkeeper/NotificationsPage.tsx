...
const ICONS: Record<TNotificationType, string> = {
  NEW_BAKI: '📤', PAYMENT_RECEIVED: '📥', REMINDER_SENT: '💬',
  SHOP_VERIFIED: '✅', REFERRAL_REWARD: '🎁', SYSTEM: 'ℹ️',
};
    ...
    <div className="p-4 animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div className="text-xs text-slate-400 uppercase tracking-wide font-semibold">
          🔔 নোটিফিকেশন
          {unread > 0 && <span className="ml-1 bg-red-600 text-white rounded-full px-1.5 py-0.5 text-[10px]">{unread}</span>}
        </div>
        {unread > 0 && (
          <button onClick={() => markAllMut.mutate()} className="text-teal-400 text-xs hover:underline">সব পড়েছি</button>
        )}
      </div>
        ...
        <div className="space-y-2">
          {notifs.map((n: any) => (
            <div key={n.id}
              ...
              className={`bg-slate-800/60 border rounded-xl p-3 flex items-start gap-3 cursor-pointer transition-all
                ${n.isRead ? 'border-slate-700/30 opacity-70' : 'border-slate-700 hover:border-teal-800'}`}>
