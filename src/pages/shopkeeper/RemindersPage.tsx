  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      <div>
        ...
        {history.length === 0 ? (
          <div className="text-center text-slate-500 py-8">কোনো reminder পাঠানো হয়নি</div>
        ) : (
              ...
              <div key={r.id} className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3">
                <div className="flex items-center justify-between mb-1">
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full
                    ${r.status === 'SENT' ? 'bg-teal-950 text-teal-400' : r.status === 'FAILED' ? 'bg-red-950 text-red-400' : 'bg-amber-950 text-amber-400'}`}>
                    {r.channel} · {r.status}
                  </span>
      ...
      <Modal open={showSend} onClose={() => { setShowSend(false); setSent(null); setErr(''); }} title="Reminder পাঠান">
        {sent ? (
          <div className="text-center space-y-4">
            ...
            {sent.whatsappLink && (
              <a href={sent.whatsappLink} target="_blank" rel="noreferrer"
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={custId} onChange={e => onCustChange(e.target.value)}
                className="w-full bg-slate-800 border border-slate-600 focus:border-teal-500 rounded-xl px-3 py-2.5 text-white text-sm outline-none">
            ...
            <div>
              ...
              <textarea value={msg} onChange={e => setMsg(e.target.value)} rows={4}
                className="w-full bg-slate-800 border border-slate-600 focus:border-teal-500 rounded-xl px-3 py-2.5 text-white text-sm outline-none resize-none" />
            </div>
