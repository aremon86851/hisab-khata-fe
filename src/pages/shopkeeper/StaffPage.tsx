...
import { EmptyState, PageLoader, Input, Button, Modal, Toggle } from '../../components/shared';
...
export default function StaffPage() {
  ...
  const [form,    setForm]    = useState<any>({
    name: '', mobile: '', pin: '', role: 'STAFF',
    canAddBaki: true, canAddPayment: true, canAddCustomer: false,
    canViewReport: false, canManageProduct: false, canSendReminder: false,
  });
  ...
  const addMut = useMutation({
    ...
    onSuccess:  () => { qc.invalidateQueries({ queryKey: ['staff'] }); setShowAdd(false); setErr(''); },
  ...
  const pinMut = useMutation({
    ...
    onSuccess:  () => { setPinFor(null); setNewPin(''); },
  });
            ...
            <div key={s.id} className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-slate-400 text-xs font-mono">{s.mobile}</div>
                  {s.lastLoginAt && <div className="text-slate-500 text-xs">সর্বশেষ: {relativeTime(s.lastLoginAt)}</div>}
                </div>
                <span className={`px-2 py-0.5 rounded-full text-xs font-bold
                  ${s.role === 'MANAGER' ? 'bg-violet-900/60 text-violet-400' : 'bg-slate-700 text-slate-300'}`}>
                  {s.role === 'MANAGER' ? 'ম্যানেজার' : 'স্টাফ'}
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5 mb-3">
                {PERMS.filter(p => s[p.k]).map(p => (
                  <span key={p.k} className="text-[10px] px-2 py-0.5 rounded-full bg-teal-950/60 text-teal-400 font-semibold">✓ {p.l}</span>
                ))}
              </div>
              <div className="flex gap-2">
                <button onClick={() => setPinFor(s)}
                  className="flex-1 text-xs py-1.5 rounded-lg border border-slate-700 text-slate-400 hover:border-teal-600 hover:text-teal-400 transition-colors">
                  🔑 PIN Reset
                </button>
                <button onClick={() => remMut.mutate(s.id)}
                  className="text-xs py-1.5 px-3 rounded-lg border border-slate-700 text-red-400 hover:border-red-600 transition-colors">
                  সরান
                </button>
              </div>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            {(['STAFF', 'MANAGER'] as const).map(r => (
              <button key={r} onClick={() => setForm((f: any) => ({ ...f, role: r }))}
                className={`flex-1 py-2 rounded-xl text-xs font-bold border transition-all
                  ${form.role === r ? 'bg-teal-600 text-white border-teal-500' : 'border-slate-700 text-slate-400'}`}>
      ...
      <Modal open={!!pinFor} onClose={() => setPinFor(null)} title={`${pinFor?.name} — PIN Reset`}>
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => pinFor && pinMut.mutate({ id: pinFor.id, pin: newPin })}
