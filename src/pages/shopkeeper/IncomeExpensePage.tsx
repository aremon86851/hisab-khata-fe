...
import { StatCard, PageLoader, Input, Button, Modal } from '../../components/shared';
...
export default function IncomeExpensePage() {
  ...
  const addMut = useMutation({
    ...
    onSuccess:  () => {
      ...
      setShowAdd(false); setForm({ type: 'INCOME', amount: '', category: '', note: '' }); setErr('');
    },
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between bg-slate-800/60 border border-slate-700 rounded-xl px-4 py-2">
        <button onClick={prevMonth} className="text-slate-400 hover:text-white text-xl">←</button>
        <span className="text-white font-bold">{banglaMonth(month)} {year}</span>
        <button onClick={nextMonth} className="text-slate-400 hover:text-white text-xl">→</button>
      </div>
      ...
      <button onClick={() => { setShowAdd(true); setForm(f => ({ ...f, type: tab, category: '' })); }}
        ...
        {(['INCOME', 'EXPENSE'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`flex-1 py-2 rounded-xl text-sm font-bold border transition-all
              ${tab === t ? (t === 'INCOME' ? 'bg-teal-600 text-white border-teal-500' : 'bg-red-600 text-white border-red-500') : 'border-slate-700 text-slate-400'}`}>
            {t === 'INCOME' ? '📈 আয়' : '📉 ব্যয়'}
          </button>
        ))}
      </div>
            ...
            {(['INCOME', 'EXPENSE'] as const).map(t => (
              <button key={t} onClick={() => setForm(f => ({ ...f, type: t, category: '' }))}
                className={`flex-1 py-2 rounded-xl text-sm font-bold border
                  ${form.type === t ? (t === 'INCOME' ? 'bg-teal-600 text-white border-teal-500' : 'bg-red-600 text-white border-red-500') : 'border-slate-700 text-slate-400'}`}>
                {t === 'INCOME' ? 'আয়' : 'ব্যয়'}
              </button>
            ))}
          </div>
              ...
              {(form.type === 'INCOME' ? INCOME_CATS : EXPENSE_CATS).map(c => (
                <button key={c} onClick={() => setForm(f => ({ ...f, category: c }))}
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all
                    ${form.category === c ? 'bg-teal-600 text-white border-teal-500' : 'border-slate-700 text-slate-400'}`}>
                  {c}
                </button>
              ))}
            </div>
