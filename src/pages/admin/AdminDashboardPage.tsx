  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      <div className="grid grid-cols-2 gap-3">
        <StatCard
          label="Total Shops"
          value={s?.totalShops || 0}
          sub={`${s?.verifiedShops || 0} verified`}
          color="teal"
        />
        <StatCard
        ...
        <StatCard
          label="মোট বকেয়া"
          value={taka(s?.totalOutstandingBaki || 0)}
          sub="outstanding"
          color="red"
        />
      </div>
        ...
        <div className="space-y-2">
          {(s?.recentTransactions || []).map((t: any) => (
            ...
            >
              ...
              <div
                className={`font-mono text-sm font-bold flex-shrink-0 ${t.type === "BAKI" ? "text-red-400" : "text-teal-400"}`}
              >
                {t.type === "BAKI" ? "+" : "−"}
                {taka(t.amount)}
              </div>
