  ...
  return (
    <div className="p-4 space-y-3 animate-fade-in">
      ...
      {verifications.length === 0 ? (
        <EmptyState
          icon="✅"
          title="সব Verification শেষ!"
          sub="কোনো pending নেই"
        />
      ) : (
        <div className="space-y-3">
          {verifications.map((v: any) => (
            <div
              key={v.id}
              className="bg-white dark:bg-slate-800/60 border border-amber-200 dark:border-amber-800/40 rounded-xl p-4"
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="bg-amber-100 dark:bg-amber-950 text-amber-600 dark:text-amber-400 text-xs px-2 py-0.5 rounded-full font-bold flex-shrink-0">
                  Pending
                </span>
              </div>
              <button
      ...
      <Modal
        ...
        title={`Review: ${selected?.shop?.name}`}
      >
        <div className="space-y-4">
          <div className="bg-gray-100 dark:bg-slate-800 rounded-xl p-3 text-sm space-y-1.5">
            {[
          ...
          <Input
            label="Review Note (প্রত্যাখ্যানের কারণ)"
            value={note}
